...
SELECT last_name, count(actor_id) AS 'Number of Actors' FROM actor 
GROUP BY last_name;
...
SELECT CONCAT(first_name,' ', last_name) AS 'Staff Member', address FROM staff s 
JOIN address a ON s.address_id = a.address_id;

#6b. Use JOIN to display the total amount rung up by each staff member in August of 2005. Use tables staff and payment.
SELECT CONCAT(s.first_name,' ', s.last_name) AS 'Staff Member', SUM(amount) AS 'Total Amount Rung Up($)' FROM staff s 
...
SELECT COUNT(inventory_id) AS 'Hunchback Impossible (# of copies)' FROM inventory 
WHERE film_id = (SELECT film_id FROM film WHERE title = 'Hunchback Impossible');

#6e. Using the tables payment and customer and the JOIN command, list the total paid by each customer. 
#List the customers alphabetically by last name:
SELECT CONCAT(c.last_name,', ', c.first_name) AS 'Customer', SUM(p.amount) AS 'Total Paid($)' FROM payment p 
...
SELECT title AS 'Movie Title' FROM film WHERE language_id IN (
...
SELECT CONCAT(first_name,' ', last_name) AS 'Customer (Canada)', email FROM customer c
	JOIN address a
...
SELECT f.title AS 'Movie Category: Family Films' FROM film f
